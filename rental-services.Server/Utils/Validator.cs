...
namespace rental_services.Server.Utils
{
    public static partial class Validator
    {
        ...
        [GeneratedRegex(@"^(?=.*\d)(?=.*[A-Z])(?=.*[a-z])(?=.*[^\w\d\s:])([^\s]){8,32}$", RegexOptions.Multiline)] 
        private static partial Regex PasswordReg();

        public static bool PhoneNumber(string phoneNumber)
        {
            return PhoneNumberReg().IsMatch(phoneNumber);
        }

        [GeneratedRegex(@"^(\+\d{2}|0)(\s|)\d{9}$")]
        private static partial Regex PhoneNumberReg();
    }
}
